...
import { fetchProfile, uploadImage, uploadProfile } from '../../store/redux/profile/ProfileRequest';
...
function Profile() {
    ...
    const fetchProfileMethod = async () => {
        const response = await fetchProfile(dispatch, jwt);
        if (response === false) {
            navigate("/auth/login")
        }
    }
    ...
    const handleUploadImage = async () => {
        try {
            ...
            if (fileAvatar) {
                ...
                if (profile?.avatar) {
                    await FirebaseFileDelete(profile.avatar);
                }
                setFileAvatar(null);
                await uploadImage(dispatch, jwt, url);
    ...
    const onChangeHandleUploadFile = (e) => {
        setFileAvatar(e.target.files[0]); // Use e.target.files[0] to get the selected file
    };
    ...
    const handleSubmitUpload = async () => {
        const response = await uploadProfile(dispatch, jwt, profileForm);
        if (response) {
            toast.success("Update information success");
            setStatusUpload(false);
        }
        else {
            toast.error("Update information faild.");
        }
    }
        ...
        <div className='profile-container'>
            ...
            <div className='left-container'>
                <div className='avatar-container'>
                    <img src={profileForm?.avatar || imgUrl} className='avatar'></img>
                    <label for="upload-photo" style={{ zIndex: "1" }} className='btn-update-image'>Upload</label>
                    <input type="file" accept="image/*" id="upload-photo" onChange={onChangeHandleUploadFile} />
                </div>
                <div className='text-container'>
                    <span className='txt-username'>{profileForm?.username}</span>
                    <span className='txt-email'>{profileForm?.email}</span>
                </div>
            </div>
            <div className='right-container'>
                <form className='form-container'>
                    ...
                    <div className='input-container'>
                        ...
                        <input className='input-style' value={profileForm?.name} disabled={!statusUpload} onChange={(e) => handleInputForm(e)}
                            name='name'></input>
                    </div>
                    <div className='input-container'>
                    ...
                    <div className='input-container'>
                        ...
                        <input type='date' className='input-style' value={profileForm?.dob} disabled={!statusUpload} onChange={handleInputForm} name='dob'></input>
                    </div>
                    <div className='input-container'>
                        <span className='label-input-tite'>Phone</span>
                        <input className='input-style' value={profileForm?.phone} name='phone' disabled={!statusUpload} onChange={handleInputForm}></input>
                    </div>
                    <div className='input-container'>
                        <span className='label-input-tite'>Address</span>
                        <input className='input-style' name='address' value={profileForm?.address} disabled={!statusUpload} onChange={handleInputForm}></input>
                    </div>
                    <div className='button-container'>
                        {statusUpload ? (
                            <div>
                                <button type='button' className='btn-update' onClick={handleSubmitUpload}>Update</button>
